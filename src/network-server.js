const log = require("./log")("network-server");
...
class NetworkServer {
  ...
  recalculateStableFrame() {
    const clientConfirmedFrames = this.clients.map(
      (client) => client.lastFrame
    );
    const serverConfirmedFrame = this.simulator.getCurrentFrame();
    const confirmedFrames = clientConfirmedFrames.concat([
...
class Client {
  ...
  constructor({ status, id, name, server, messenger, lastFrame }) {
    ...
    messenger.on("message", this.handleMessage.bind(this));
    messenger.on("close", this.handleClose.bind(this));
  }
  broadcast(msg) {
  ...
  handleSyn(msg) {
    ...
    const stableStateHash = hash(stringify(stableState));
    this.messenger.send({
      type: "ack",
      oframe: msg.frame,
      ...
      stableState: stringify(stableState),
      stableStateHash,
    });
  }
  ...
  handleClose() {
    const simulator = this.server.simulator;
    simulator.pushEvent({
      type: "disconnect",
      clientid: this.id,
    });
