const log = require("./log")("network-client");
...
class NetworkClient {
  constructor({ messenger, simulator }) {
    ...
    this.messenger.on("message", this.handleMessage.bind(this));
  ...
  handleAck(msg) {
    ...
    this.serverFrame = msg.nframe;
    this.framesDifference = framesDifference;
    ...
    // This is mostly for debugging purposes, as we usually do not want to sent
    // the full state every sync.
    if (msg.stableState) {
      const serverState = msg.stableState;
      const clientState = stringify(
        this.simulator.getMoment(msg.stableFrame).state
      );
